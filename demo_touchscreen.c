...
void TS_Init(void)
{
	...
	IOE_Config();
	ts_coeff = (TS_COEFF){-0.092257,0.068120,347.973633,-17.670300};
}
...
running_e TS_Calibration(bool_e ask_for_finish, TS_Calibration_mode_e mode)
{
	...
	case COMPUTE:
		...
		if(mode >= CALIBRATION_MODE_CALIBRATE_AND_SHOW_VALUE)
		{
			...
			LCD_DisplayStringLine(LINE(3),COLUMN(0),(uint8_t *)text,LCD_COLOR_BLACK, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);
			sprintf((char*)text,"Second cross x = %d  ", second_cross_point.x);
			LCD_DisplayStringLine(LINE(5),COLUMN(0),(uint8_t *)text,LCD_COLOR_BLACK, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);
			sprintf((char*)text,"First cross y = %d  ", first_cross_point.y);
			LCD_DisplayStringLine(LINE(4),COLUMN(0),(uint8_t *)text,LCD_COLOR_BLACK, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);
			sprintf((char*)text,"Second cross y = %d  ", second_cross_point.y);
			LCD_DisplayStringLine(LINE(6),COLUMN(0),(uint8_t *)text,LCD_COLOR_BLACK, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);

			LCD_DisplayStringLine(LINE(8),COLUMN(0),(uint8_t *)"Toucher pour continuer !",LCD_COLOR_BLACK, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);
			...
			LCD_DisplayStringLine(LINE(10),COLUMN(0),(uint8_t *)text,LCD_COLOR_BLACK, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);
			...
			LCD_DisplayStringLine(LINE(11),COLUMN(0),(uint8_t *)text,LCD_COLOR_BLACK, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);
			...
			LCD_DisplayStringLine(LINE(12),COLUMN(0),(uint8_t *)text,LCD_COLOR_BLACK, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);
			...
			LCD_DisplayStringLine(LINE(13),COLUMN(0),(uint8_t *)text,LCD_COLOR_BLACK, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);

			if(mode == CALIBRATION_MODE_CALIBRATE_AND_SHOW_VALUE_AND_PROVIDE_TELECRAN_GAME)
			{
				LCD_DisplayStringLine(LINE(15),COLUMN(0),(uint8_t *)"To quit the Telecran Game,",LCD_COLOR_RED, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);
				LCD_DisplayStringLine(LINE(16),COLUMN(0),(uint8_t *)"Just press the blue button",LCD_COLOR_RED, LCD_COLOR_WHITE,LCD_NO_DISPLAY_ON_UART);
			}
		}
