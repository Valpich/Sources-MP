...
static void ETH_MACDMA_Config(void) {
	...
	while (ETH_GetSoftwareResetStatus() == SET)
	{
		...
		{
			printf("Timeout waiting software reset of Ethernet. Check the baseboard is plugged?\n");
			break;
		}
	}
...
void ETH_GPIO_Config(void)
{
	...
	BSP_GPIO_PinCfg(GPIOA, GPIO_PIN_8, GPIO_MODE_AF_PP, GPIO_NOPULL, GPIO_SPEED_FAST, 0);
	...
	*(__IO uint32_t *) PMC_MII_RMII_SEL_BB = SYSCFG_ETH_MediaInterface_MII;
#else
	/* RMII Media interface selection */
	*(__IO uint32_t *) PMC_MII_RMII_SEL_BB = SYSCFG_ETH_MediaInterface_RMII;
#endif
	/* Check if user has defined it's own pins */
	if (!TM_ETHERNET_InitPinsCallback()) {
#ifdef ETHERNET_MII_MODE
		/* GPIOA                     REF_CLK      MDIO         RX_DV */
		BSP_GPIO_PinCfg(GPIOA, GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_7								, GPIO_MODE_AF_PP, GPIO_NOPULL, GPIO_SPEED_HIGH, GPIO_AF11_ETH);
		/* GPIOB                     PPS_PUT      TDX3 */
		BSP_GPIO_PinCfg(GPIOB, GPIO_PIN_5 | GPIO_PIN_8											, GPIO_MODE_AF_PP, GPIO_NOPULL, GPIO_SPEED_HIGH, GPIO_AF11_ETH);
		/* GPIOC                     MDC          TDX2         TX_CLK       RXD0         RXD1 */
		BSP_GPIO_PinCfg(GPIOC, GPIO_PIN_1 | GPIO_PIN_2	| GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5	, GPIO_MODE_AF_PP, GPIO_NOPULL, GPIO_SPEED_HIGH, GPIO_AF11_ETH);
		/* GPIOG                     TX_EN         TXD0          TXD1 */
		BSP_GPIO_PinCfg(GPIOG, GPIO_PIN_11 | GPIO_PIN_13	| GPIO_PIN_14						, GPIO_MODE_AF_PP, GPIO_NOPULL, GPIO_SPEED_HIGH, GPIO_AF11_ETH);
		/* GPIOH                     CRS          COL          RDX2         RXD3 */
		BSP_GPIO_PinCfg(GPIOH, GPIO_PIN_2 | GPIO_PIN_3	| GPIO_PIN_6 | GPIO_PIN_7				, GPIO_MODE_AF_PP, GPIO_NOPULL, GPIO_SPEED_HIGH, GPIO_AF11_ETH);
		/* GPIOI                     RX_ER */
		BSP_GPIO_PinCfg(GPIOI, GPIO_PIN_10 														, GPIO_MODE_AF_PP, GPIO_NOPULL, GPIO_SPEED_HIGH, GPIO_AF11_ETH);
#else	/* RMII */
		/* Init default pins */
		...
		/* RX pins and MDC */
		BSP_GPIO_PinCfg(GPIOC, GPIO_PIN_1 | GPIO_PIN_4 | GPIO_PIN_5, GPIO_MODE_AF_PP, GPIO_NOPULL, GPIO_SPEED_HIGH, GPIO_AF11_ETH);
		...
		BSP_GPIO_PinCfg(GPIOB, GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_13, GPIO_MODE_AF_PP, GPIO_NOPULL, GPIO_SPEED_HIGH, GPIO_AF11_ETH);
#endif /* RMII pinspack 2 */
#endif /* ETHERNET_USE_MII */

		BSP_GPIO_PinCfg(GPIOE, GPIO_PIN_2, GPIO_MODE_OUTPUT_PP, GPIO_PULLUP, GPIO_SPEED_FAST, 0);
